function spectrum_2d_kx_kz(data)
InputValues = inputdlg({'Work Function(eV)','Incidence Angle(degrees)','Inner Potential(eV)'},'',1,{'5','20','10'});
if isempty(InputValues)==1
    return
else
    w=str2double(InputValues{1});
    inc=str2double(InputValues{2});
    inner_poten=str2double(InputValues{3});
    ...
    for k=1:length(data)
        ...
        k_y{k} = 0.5124.*(sqrt((y-w).*cosd(x).*cosd(x)+inner_poten))-hv_p*sind(inc);
        clear x y e_p hv_p
