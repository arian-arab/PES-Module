function spectrum_2d_multiply_spectrum(data)
for k=1:length(data)
...
multiply = data{1}.c_data-data{1}.c_data+1;
for k=1:length(data)
    multiply = multiply.*data{k}.c_data;
end
data_add{1}.x_data = data{1}.x_data;
data_add{1}.y_data = data{1}.y_data;
data_add{1}.c_data = multiply;
data_add{1}.name='multiplied spectrum';
